...
class LPBlurDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.opt.mode == 'train':
            ...
            #print(f"Plate info for {os.path.basename(self.sharp[idx])}: {plate_info}") 
            plate_info_str = self.txt[os.path.basename(self.sharp[idx])]
            # 将车牌信息字符串转换为固定长度的向量
            plate_info_num = [self.char_to_num[char] for char in plate_info_str]
            plate_info_num += [self.char_to_num['#']] * (self.vector_length - len(plate_info_num))
            plate_info = torch.tensor(plate_info_num, dtype=torch.float)
            #try:
